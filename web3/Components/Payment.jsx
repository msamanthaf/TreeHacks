...
const Payment = ({ setOpenModel, pay, payFunction }) => {
  const [amount, setAmount] = useState("");

  const createPay = async () => {
    try {
      const data = await payFunction(pay.finder, amount);
      console.log(data);
        ...
        <div className="relative w-auto my-6 mx-auto max-w-3xl">
          ...
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
			...
			<p className="text-black text-lg leading-relaxed">
                {pay.targetName}, {pay.targetAge}
              </p>
			</div>
            <div className="relative px-6 flex-auto">
              <p className=" text-slate-500 text-lg leading-relaxed">
                by {deprecateString(pay.finder)}
              </p>
            ...
            <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
              ...
              <button
                className="background text-white bg-emerald-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
                type="button"
                onClick={() => createPay()}
              >
                Donate
